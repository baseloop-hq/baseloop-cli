...
func TestAuthLoginManualPasteCode(t *testing.T) {
	...
	oldStdin := stdin
	// Paste a callback URL, the forgiving path a user hits when the hosted
	// page is unavailable and they copy the address bar instead.
	stdin = strings.NewReader(server.URL + "/cli/callback?code=pasted-code\n")
...
func TestAuthLoginManualRejectsForeignState(t *testing.T) {
	var tokenCalls int
	var server *httptest.Server
	mux := http.NewServeMux()
	mux.HandleFunc("/.well-known/oauth-authorization-server", func(w http.ResponseWriter, r *http.Request) {
	...
	t.Setenv("BASELOOP_CONFIG", filepath.Join(t.TempDir(), "config.json"))
	oldStdin := stdin
	// A URL carrying someone else's state must not be exchanged.
	stdin = strings.NewReader(server.URL + "/cli/callback?code=attacker-code&state=not-ours\n")
	t.Cleanup(func() { stdin = oldStdin })
	...
	if !strings.Contains(out.String(), "different login attempt") {
		t.Errorf("expected a state mismatch error, got %s", out.String())
	}
	if tokenCalls != 0 {
		t.Errorf("expected no token exchange, got %d", tokenCalls)
	}
}
