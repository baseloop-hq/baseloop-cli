...
`scripts/release.sh VERSION` requires a clean tree, runs `make release-check`, builds locally as a smoke test, then tags `vVERSION` and pushes; the workflow in `.github/workflows/release.yml` builds every platform, generates the pinned installer assets, and publishes. A hyphenated version (`1.2.3-rc.1`) publishes as a prerelease that GitHub's "latest" pointer, the installers, and `baseloop upgrade` all ignore, so anything touching the installer or update path should ship as an rc first. `scripts/verify-release.sh VERSION` waits for the workflow and then checks the published result: run success, prerelease flag, "latest" pointer, all assets present, installer stamps, checksum of this platform's archive, and a sandboxed install from the published `install-cli` that must report the version and record a `managed` receipt. Promote by tagging the final version on the same commit and verifying again. The `release-cli` skill (`.claude/skills/release-cli/SKILL.md`) is the agent-facing walkthrough of this sequence.
...
- Records installer-owned local state in an install manifest at `~/.local/state/baseloop/manifest.json` (or `$XDG_STATE_HOME/baseloop/`). Both installers have the freshly extracted binary install itself via the hidden `baseloop setup install --source <file> --target <file> --policy <pinned|managed>` (`pinned` when the operator set `BASELOOP_VERSION`, `managed` otherwise), which takes the same `upgrade.lock` the background updater uses, records the receipt, and swaps the binary inside that one critical section: an updater already mid-flight finishes first, one that starts later sees the pin and stands down, and a failed swap restores the previous policy. That invocation runs with `BASELOOP_NO_UPDATE_CHECK=1` (and the CLI itself never starts an update check for `setup install` or `setup receipt`), so the temporary binary cannot spawn an updater of its own. When the requested release predates the command (a downgrade), the installers retry `setup install` through the binary currently installed, which then takes the lock, records the pin, and replaces itself with the older release in one critical section. Only when nothing on the machine knows the command (no install, or one older than the receipt itself, where no pin-aware updater can exist) do they fall back to a plain move that the installer performs under the same `upgrade.lock`, created exclusively with the installer's live PID exactly as the CLI writes it (waiting up to `BASELOOP_INSTALL_LOCK_WAIT` seconds, default 120, and taking over a lock older than the CLI's 10-minute stale timeout), followed by the hidden `baseloop setup receipt --policy <pinned|managed>`; a pinned install gets no update notices, never background-updates, and doctor's `cli_version` advisory stays `ok` (with the newer release named in the hint) so agent workflows never upgrade it as a side effect. A successful manual `baseloop upgrade` releases the pin back to `managed` — including when it lands on the version already installed — while a background update can never release a pin. Receipt writing is best-effort: it must not fail an otherwise working install.
